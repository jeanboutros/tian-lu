...
- `setup-floci.sh` — main installer script (996 lines, implemented). Creates `floci` user, installs rootless Podman, configures Floci container with persistent storage + TLS, systemd user service, UFW firewall rules. Idempotent. Seven phases: preflight → user setup → Podman setup → network & image → Floci config → start & verify → summary.
...
- `docs/design/` — design documents:
  ...
  - `authentication-plan.md` — authoritative auth design: env=account (12-digit AKID), account-root deployment, `FLOCI_AUTH_MODE`, per-env credentials, and the F-01 signature-validation known issue.
...
- `scripts/pre-commit` — pre-commit hook (lint + unit tests); install with `make install-hooks` (or `cp scripts/pre-commit .git/hooks/pre-commit && chmod +x .git/hooks/pre-commit`).
- `scripts/pre-push` — pre-push hook (runs `make ci-test`, the GitHub Actions reproducer, and blocks the push on Linux-only test failures); install with `make install-hooks`.
...
- `mock-server/lima/floci-dev.yaml` — Lima template for the persistent dev VM (Ubuntu 26.04 arm64 QEMU; repo root mounted **read-only** at `/opt/tianlu` over 9p, `dev-twin.sh` supplying the host path via `--set='.mounts[0].location=…'`; standalone `floci-dev-data` disk at `/mnt/lima-floci-dev-data`; all service ports forwarded to `127.0.0.1`, except Lambda Runtime API ports 9200-9299 which are **not** forwarded). Declaring `mounts:` replaces Lima's defaults, so the guest has no home mount and no writable host mount.
- `infra/` — **secondary project**: an educational AWS landing zone in Terraform that runs *on* Floci. Stages: `00-backend-bootstrap` (S3+DynamoDB state) → `10-management-iam` (platform-admin + permissions boundary) → `20-network-hub` → `30-platform-eks` (k3s) → `40-app-alpha` (workload spoke + FastAPI) → Phase 2 (`50-app-beta`, `60-spoke-to-spoke`). See `infra/README.md` and `docs/design/landing-zone-design.md`. Run `scripts/preflight-floci.sh` (G1/G2/G3 gates) before any `terraform apply`. See `infra/AGENTS.md` for stage conventions, backend wiring, and the IAM delegation model.
- `docs/learning/` — **isolated** Socratic research loop (`/learn <question>`); produces ADRs (`decisions/NNNN-*.md`), gap findings (`findings/NNNN-*.md`), session transcripts (`sessions/YYYY-MM-DD-*.md`), and an append-only `journal.md`. Written ONLY by the `/learn` command — never edit manually, never mix with installer work. See `docs/learning/README.md` and `docs/learning/AGENTS.md` for the round-shape checklist and isolation rules.
- `docs/superpowers/` — meta-design layer for the learning system: `specs/` (design specs), `plans/` (implementation plans with checkbox tracking), `WORKTREE-GUIDE.md` (Git worktree guide for isolating learning-loop work from installer branches).
- `.github/agents/` — subagent definitions for the learning loop (`learn-challenger.md`, `learn-researcher.md`, `learn-synthesizer.md`); invoked by the `/learn` command (`.github/commands/learn.md`, `.github/prompts/learn.prompt.md`).
- `.github/workflows/opencode.yml` — opencode agent trigger workflow (issue/PR comment events).
- `.github/instructions/mermaid.instructions.md` + `.github/copilot-instructions.md` — VS Code Mermaid extension AI skills (diagram validation/preview rules).
- `scripts/preflight-floci.sh` — pre-flight gates for the `infra/` Terraform project. Verifies Floci actually enforces IAM signatures (G1), RDS IAM auth (G2 manual notes), DynamoDB conditional-write locking (G3), k3s (G4/G5 notes). G1 is a hard stop — without signature validation the IAM lessons are false demos.
- `scripts/help.sh` — `make help` text printer.
...
- **The Quadlet must set `UserNS=keep-id:uid=1001,gid=1001`.** The Floci image runs as container uid 1001 (gid 0). Rootless Podman's default mapping maps the host `floci` user to container root (uid 0), so the bind-mounted data directory is root-owned inside the container and the container's `floci` user (1001) cannot write to `/app/data` → `java.nio.file.AccessDeniedException: /app/data/tls` and the service fails to start. `keep-id:uid=1001,gid=1001` maps the host `floci` user to container uid 1001 instead, keeping the host directory owned by `floci` while making it writable inside. The mapping names the container-side uid, so it is independent of whichever host uid `useradd` assigned to `floci`.
...
- **The script must poll for the user manager after `loginctl enable-linger`.** `enable-linger` starts the manager asynchronously. Poll `systemctl is-active --quiet user@<UID>.service` then `run_as_floci systemctl --user is-active --quiet default.target`. Do NOT use `is-system-running` (fails on `degraded`).
...
- **The pinned-user preflight fails on an instance created from a different `user:` block.** Lima applies `user:` at creation time only, so a pre-existing `floci-twin`/`floci-dev` instance keeps whatever username and uid it was built with. Recreate it (`make twin-test TWIN_FLAGS="--fresh --destroy"`, or `make dev-recreate`) — the default `--keep` will keep failing the check.
...
- **The dev twin disables TLS (`FLOCI_TLS_ENABLED=false`).** The production installer keeps TLS on (`FLOCI_TLS_ENABLED=true`, self-signed cert); the dev twin overrides it to `false` where it invokes the installer (`dev-twin.sh:_install_absent`). The Floci UI sidecar's Node backend rejects the self-signed cert, so with TLS off Floci serves plain HTTP on 4566 and the built-in UI launcher injects `http://tianlu-floci:4566`.
...
Applies to every code comment, `AGENTS.md`, and Markdown document in this repository.

- **Document the current state only.** Describe what the code or configuration does now. Do not record what it used to do, what changed, or why it changed — that history belongs in the git log.
- **When behaviour changes, replace the text.** Delete the superseded wording and write the new behaviour in its place. Do not append a note, a changelog entry, or a "previously…" clause.
- **Explain the why.** State the constraint that makes a value, flag, or approach necessary — the upstream behaviour, kernel restriction, or failure mode it prevents. Add the *what* only where the mechanism is non-obvious.
- **Be factual and direct.** Plain technical language, no metaphors, no marketing terms, no filler.
- **Cite the constraint, not the anecdote.** Point at the man page, upstream issue, or spec that makes the requirement true.
...
- **Run after every change to `setup-floci.sh`:** `make check` (fast, local) before commit, `make ci-test` (GitHub Actions reproducer) + `make twin-test` before push. The pre-commit hook (`scripts/pre-commit`) runs lint+unit automatically; the pre-push hook (`scripts/pre-push`) runs `make ci-test` and blocks the push on Linux-only test failures (install both with `make install-hooks`); CI (`.github/workflows/test.yml`) runs lint+unit on every push/PR. Run `make twin-test` locally before pushing installer changes. See `docs/testing-guide.md` for the full guide.
