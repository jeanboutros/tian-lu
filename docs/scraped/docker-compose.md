...
The default configuration works out of the box for most services: SQS, SNS, S3, DynamoDB, SSM, Lambda, API Gateway, Cognito, KMS, Kinesis, Secrets Manager, CloudFormation, Step Functions, IAM, STS, EventBridge, Scheduler, and CloudWatch.
...
> Docker socket: Lambda, ElastiCache, RDS, OpenSearch, and MSK require access to the Docker socket (`/var/run/docker.sock`) to spawn and manage containers. If you don't use these services, you can omit that volume.

> ECR port: ECR is backed by a `registry:2` sidecar container (`floci-ecr-registry`) that Floci starts and manages. That container binds its own host port (default `5100`) directly — do not add `5100-5199` to the Floci service's `ports` list.
...
services:
  floci:
    ...
    ports:
      - "4566:4566"
    environment:
      FLOCI_HOSTNAME: floci   # Must match the Compose service name so other containers can resolve it by DNS.
...
Floci automatically attaches Lambda containers it spawns to the same Compose network when no explicit Docker network is configured. Setting `FLOCI_HOSTNAME` ensures those containers receive a reachable endpoint.
...
> CI pipelines: In GitHub Actions or GitLab CI where both your app and Floci run as `services`, set `FLOCI_HOSTNAME` to the service name (e.g. `floci`) and point your SDK at `http://floci:4566`.
...
LocalStack-compat paths also work:
...
|---|---|---|
| `FLOCI_HOSTNAME` | *(none)* | Hostname embedded in response URLs. Set to the Compose service name in multi-container setups |
...
| `FLOCI_SERVICES_DOCKER_NETWORK` | *(none)* | Docker network for spawned containers (Lambda, ElastiCache, RDS, OpenSearch, MSK) |
...
For Docker daemon socket, private registry authentication, log rotation, and network settings see Docker Configuration.