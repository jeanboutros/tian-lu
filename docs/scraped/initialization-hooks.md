...
Floci supports init hook scripts that run at defined points in the startup and shutdown lifecycle. Use them to seed resources, configure state, or clean up after a run — before or after the AWS APIs are available.

> Use the compat image for scripts that call `aws` or `boto3`: Scripts that invoke the AWS CLI or Python boto3 require the compat image, which bundles Python 3, the AWS CLI, and boto3 — all pre-configured for `http://localhost:4566`. Use `floci/floci:latest-compat` (or a pinned `x.y.z-compat`) instead of the standard image.
...
The LocalStack-compat paths let existing LocalStack bootstrap scripts work without modification. Mount them under `/etc/localstack/init/` and they run as-is.
...
Within each phase, scripts run in **lexicographical order** and **sequentially** (one at a time). Prefix filenames with numbers to control order: `01-`, `02-`, `03-`, etc.
...
The compat image (`floci/floci:latest-compat`) includes the AWS CLI and boto3 with the local endpoint pre-configured. Scripts can call `aws` directly — no `--endpoint-url` flag needed:
...
Environment variables pre-set in the compat image:
...
`/etc/floci/init/ready.d/01-seed.sh`:
...
`/etc/floci/init/ready.d/01-seed.py`:
...
`/etc/floci/init/stop.d/01-cleanup.sh`:
...
> Shutdown timing: Stop hooks run before the HTTP server shuts down, so Floci's total shutdown time grows by the cumulative runtime of all stop hooks. Adjust your orchestrator grace period accordingly (e.g. Kubernetes `terminationGracePeriodSeconds`, Docker Compose `stop_grace_period`).
