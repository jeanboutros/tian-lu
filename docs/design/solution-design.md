...
`NoNewPrivileges`, `RestrictAddressFamilies`, `LockPersonality`, `RestrictRealtime`, `SystemCallArchitectures=native`, plus `Restart=on-failure`, `RestartSec="5 10 15 20 30"` (progressive backoff) and a start limit of 8 per 120s (`StartLimitBurst=8`, `StartLimitIntervalSec=120`). The start-limit window intentionally exceeds the cumulative backoff so the sliding-window counter does not latch mid-sequence; the parameters are sized to give the cold-boot AppArmor `newuidmap`/`newgidmap` profile-attach race (~25s per `digital-twin-findings.md §9`) multiple chances to settle before the service latches. Only the seccomp-based directives that do not require namespace creation or SUID stripping are kept.
