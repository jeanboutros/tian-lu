...
- **subuid/subgid:** `configure_subuid_subgid` allocates a contiguous range of `SUBUID_COUNT` (262144) starting at the first free offset at or above `SUBUID_START` (100000), scanning `/etc/subuid` and `/etc/subgid` for overlaps with existing users. It runs only when `floci` has no entry in the file. On distributions whose `useradd` allocates a range itself — Ubuntu assigns 65536 at user-creation time — the entry already exists and the installer keeps it. The correctness constraint is met either way: ranges never overlap between users, which is what `newuidmap` requires.

  Range *size* is not a correctness property. Under rootless Podman's default mapping all containers share the user's single range, so the size governs per-container UID diversity rather than container count; the 262144 figure is headroom for `--userns=auto`, which allocates a disjoint slice per container. There is no kernel or `newuidmap` cost for one large entry — the `subuid(5)` performance note concerns the number of *lines* in the file, not the size of a range — and no security benefit, since every mapped UID is unprivileged. A host with a tightly packed `/etc/subuid` can safely run a smaller range; see GAP-018 in `gaps-register.md` for the `--userns=auto` consequence.
...
`NoNewPrivileges`, `RestrictAddressFamilies`, `LockPersonality`, `RestrictRealtime`, `SystemCallArchitectures=native`, plus `Restart=on-failure` with the systemd 254+ progressive exponential backoff API: `RestartSec=5`, `RestartSteps=5`, `RestartMaxDelaySec=30` (produces geometric intervals `5s, 7s, 10s, 15s, 21s, 30s, 30s, 30s`). A start limit of 8 per 180s (`StartLimitBurst=8`, `StartLimitIntervalSec=180`) prevents crash loops. The start-limit window intentionally exceeds `StartLimitBurst × RestartMaxDelaySec` so the sliding-window counter does not latch mid-sequence; the parameters are sized to give the cold-boot AppArmor `newuidmap`/`newgidmap` profile-attach race (~25s per `digital-twin-findings.md §9`) multiple chances to settle before the service latches. Only the seccomp-based directives that do not require namespace creation or SUID stripping are kept.
